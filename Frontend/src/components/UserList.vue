...
<div class="grid grid-rows-6 space-y-4">
    <div class="bg-white p-4 rounded-2xl" v-for="list in userList.content">
        <div class="grid grid-cols-4 font-bold text-black text-xl">
          ...
          <router-link
            ...
            <button @click="userId = list.id, deletePopUp = true"><svg width="50" height="50" viewBox="0 0 512 512" >
    ...
    <div v-show="deletePopUp">
      <DeleteConfirmation @toggle="deletePopUp = false" @delete="$emit('delete',userId)"/>
    </div>
</div>
</template>
