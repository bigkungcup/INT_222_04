...
//edit event
const saveEvent = async (editEvent, displayEvent) => {
  ...
  <div class="bg-fixed bg-detail bg-no-repeat bg-auto bg-cover bg-center h-screen w-screen pt-12 px-36 pb-36">
    <div class="w-full h-full overflow-auto">
      <div class="grid grid-cols-2 text-4xl gap-y-10 break-all">
        ...
        <div class="flex col-span-2 pl-10 pr-80" v-show="popUp">
          Date/Time :
          <Datepicker :minDate="new Date()"
            :minTime="{ hour: new Date().getHours(), minutes: new Date().getMinutes() }"
    ...
    <div class="flex text-white justify-center text-2xl" v-show="popUp">
      <button class="bg-red-500 rounded-3xl w-36 py-2 mx-2 drop-shadow-xl" @click="reset(), popUp = false">
        Cancel
      </button>
      <button class="bg-green-500 rounded-3xl w-36 py-2 mx-2 drop-shadow-xl" @click="saveEvent(editEvent, displayEvent)">
        Save
      </button>
    </div>
    ...
    <div class="grid grid-rows-3.5 bg-white w-2/6 h-80 place-self-center rounded-3xl">
      ...
      <div class="grid place-items-center">
        <button class="text-4xl px-5 text-white bgPopUp rounded-3xl w-36 py-2 mx-2" @click="reset(), editPopUp = false,popUp = false">
          OK
        </button>
      </div>
    </div>
