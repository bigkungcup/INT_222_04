...
<template>
   <div class="bg-fixed bg-user bg-no-repeat bg-auto bg-cover bg-center h-screen w-screen pt-12 px-36 pb-36">
    ...
    <p v-if="showNameSame && editUser.name !== '' && editUser.name.length <= 100" class="text-lg text-red-500 absolute">*User name is not unique.</p></p>
      ...
      <p v-if="showEmailSame && editUser.email !== '' && editUser.email.match(validEmail) !== null && editUser.email.length <= 50" class="text-lg text-red-500 absolute">*User email is not unique.</p></p>
        ...
        <span class="space-x-4 text-rose-800 text-4xl mt-2 ml-6">
            <input type="radio" id="1" value="student" v-model="editUser.role" v-bind:checked="displayUser.role =='student'"/>
            <label for="1" >Student</label>

            <input type="radio" id="2" value="lecturer" v-model="editUser.role" v-bind:checked="displayUser.role =='lecturer'"/>
            <label for="2">Lecturer</label>

            <input type="radio" id="3" value="admin" v-model="editUser.role" v-bind:checked="displayUser.role =='admin'"/>
